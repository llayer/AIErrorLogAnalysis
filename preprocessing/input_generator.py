...
def create_input( path_actionshist, path_tokens, name = '', 
                  store = True, store_path = '/eos/user/l/llayer/AIErrorLogAnalysis/data/input/'):
    ...
    # drop the message
    tokens = tokens.drop(['error_msg'], axis = 1)
    #tokens['error_type'] = tokens['error_type'].apply(clean_error_type)
    tokens['avg'] = tokens['avg'].apply(lambda x:list( float(entry) for entry in x ))
    ...
    sparse_frame = sparse_frame.groupby(['task_name', 'label'], as_index=False)['error', 'site', 'site_state', 'count',
                                                                                'msg_encoded','exit_code', 'error_type',
                                                                                'avg'].agg(lambda x: list(x))
    ...
    if store == True:
        ...
        outfile = store_path + 'input_' + name + '.h5'
        sparse_frame.to_hdf( outfile , 'frame', mode = 'w')
        site_frame.to_hdf( outfile , 'frame2', mode = 'a')
        codes_frame.to_hdf( outfile, 'frame3', mode = 'a')
        msg_error_frame.to_hdf( outfile, 'frame4', mode = 'a')
        msg_sites_frame.to_hdf( outfile, 'frame5', mode = 'a')
